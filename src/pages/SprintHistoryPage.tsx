...
import { Calendar, CheckSquare, Clock, Target, ArrowRight, MessageSquare, TrendingUp } from 'lucide-react';
...
export const SprintHistoryPage: React.FC = () => {
  ...
  const fetchSprints = async () => {
    try {
      const sprintsData = await tasksService.getSprints();
      setSprints(sprintsData.sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime()));
    } catch (error) {
  ...
  const fetchSprintTasks = async (sprint: Sprint) => {
    setTasksLoading(true);
    try {
      const tasks = await tasksService.getTasksBySprintId(sprint.id);
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center space-x-4 mb-6">
            ...
            <div>
              ...
              <p className="text-lg text-gray-600 mt-2">Manage and review your project sprints</p>
            </div>
          </div>
        </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
              ...
              <div className="p-4 space-y-3 max-h-[600px] overflow-y-auto">
                {sprints.map((sprint) => {
                  ...
                  const totalTasks = sprintTasks.filter(t => t.sprintId === sprint.id).length;
                  
                  return (
                    <button
                      key={sprint.id}
                      onClick={() => fetchSprintTasks(sprint)}
          ...
          <div className="lg:col-span-8">
            {!selectedSprint ? (
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-12 text-center">
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-3">Select a Sprint</h3>
                <p className="text-gray-600 text-lg">Choose a sprint from the list to view its tasks and details</p>
              </div>
            ) : (
